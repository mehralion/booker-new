 ...
 */
class BaseRatioX2 extends BaseRatio implements iRatio
{
    /**
     * @return int
     */
    public function check()
    {
        ...
        $msg .= \CHtml::tag('li', [], sprintf('Операция: %s >= %s = %s',
            ...
            $this->getEvent()->getResult()->getTeam2Result() >= $this->getEvent()->getResult()->getTeam1Result() ? ' TRUE' : 'FALSE'
        ));

        if($this->getEvent()->getResult()->getTeam2Result() >= $this->getEvent()->getResult()->getTeam1Result()) {
