 ...
 */
class BaseRatioPlus151 extends BaseRatio implements iRatio
{
    /**
     * @return int
     */
    public function check()
    {
        ...
        $msg .= \CHtml::tag('li', [], sprintf('Операция: %s + 1.5 > %s = %s',
            $this->getEvent()->getResult()->getTeam1Result(),
            $this->getEvent()->getResult()->getTeam2Result(),
            $this->getEvent()->getResult()->getTeam1Result() + 1.5 > $this->getEvent()->getResult()->getTeam2Result() ? 'TRUE' : 'FALSE'
        ));

        if($this->getEvent()->getResult()->getTeam1Result() + 1.5 > $this->getEvent()->getResult()->getTeam2Result()) {
